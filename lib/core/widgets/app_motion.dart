...
extension MotionX on Widget {
  ...
  Widget fadeSoft({Duration? delay, Duration? duration}) =>
      animate().fadeIn(delay: delay, duration: duration ?? Motion.slow, curve: Motion.ease);

  /// Fade up — the default entrance for headings, copy and form rows.
  Widget fadeSlideUp({Duration? delay, double offset = 0.14, Duration? duration}) => animate()
      .fadeIn(delay: delay, duration: duration ?? Motion.base, curve: Motion.ease)
      .slideY(begin: offset, end: 0, delay: delay, duration: duration ?? Motion.base, curve: Motion.ease);

  /// Fade in from the left/right, for artwork anchored to a screen edge.
  Widget fadeSlideX({Duration? delay, double offset = 0.18, Duration? duration}) => animate()
      .fadeIn(delay: delay, duration: duration ?? Motion.slow, curve: Motion.ease)
      .slideX(begin: offset, end: 0, delay: delay, duration: duration ?? Motion.slow, curve: Motion.ease);

  /// Settle in with a gentle scale — cards, badges, the splash mark.
  Widget popIn({Duration? delay, double from = 0.92, Duration? duration}) => animate()
      .fadeIn(delay: delay, duration: duration ?? Motion.base, curve: Motion.ease)
      .scale(
  ...
  Widget drift({double dy = 6, int seconds = 4, Duration? delay}) => animate(
